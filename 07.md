...
It then mints a short-lived token scoped to joining that one room, under a **random SFU identity** it assigns to the participant, carried inside the token and echoed as the response's `identity` so the client can announce it in presence (§4). The identity MUST carry at least **128 bits of fresh entropy**: it feeds the per-sender key derivation (§3), so two participants colliding on one identity would share one frame key and one IV space — recreating exactly the nonce-domain collision §3 exists to prevent (and an SFU treats identity reuse as the same participant reconnecting, kicking the first). Only a holder of the Channel's key can derive `voice_key.sk`, so the broker needs no lookup and no Community knowledge: it cannot tell which Community a room belongs to, and it never learns who is joining.
...
where `identity` is that publisher's broker-assigned SFU identity (§2), the label rides the frozen registry (CORD-02 A.6), and the epoch field is omitted, `voice_media_key` already carries it. The `sender_key` is that publisher's per-sender key **material**: frames are encrypted with **AES-256-GCM** keyed from it — under the raw 32 bytes directly, or through the media layer's own *fixed, deterministic* KDF when the client rides an established frame-encryption layer (e.g. an SFrame-style insertable-streams implementation that derives its frame key from the material it's handed). Either way the keying stays strictly per-sender, an IV MUST never repeat under one derived frame key (a per-sender monotonic counter suffices), and every client in a room MUST use the same media layer, or frames simply don't decode. A frame layer MAY leave the few codec header bytes an SFU needs for routing unencrypted — that is metadata, never content. A receiver derives each publisher's key from the identity the SFU presents for the track, so every member computes every key and there is no in-band exchange.
